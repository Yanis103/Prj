...
public class AppVueData {
    ...
    private final String[] columnNames = {"Nom Etudiant", "Prénom Etudiant", "Nom Formation","Binôme Référence","Nom Matière", "Sujet","Date Remise Prévue","Date Remise Effective","Note Rapport", "Note Soutenance", "Note Finale"};
    ...
    public AppVueData() {
        ...
        for (int i = 0; i < vueDataList.size(); i++) {
            ...
            data[i][3] = vueData.getBinomeReference();
            data[i][4] = vueData.getNomMatiere();
            data[i][5] = vueData.getSujet();
            data[i][6] = vueData.getDateRemisePrevue();
            data[i][7] = vueData.getDateRemiseEffective();
            data[i][8] = vueData.getNoteRapport();
            data[i][9] = vueData.getNoteSoutenance();
            data[i][10] = String.format("%.2f", vueData.getNoteFinale());
        }
